...
Personal dotfiles for Apple Silicon macOS and Debian servers. Symlinks are managed by `./install.sh` plus Bash modules in `install/`; there is no GNU Stow. Repo layout mirrors `$HOME` directly: dotfiles live at repo root (`.zshrc`, `.claude/`, `.agents/`, `.config/<pkg>/`), so each link-table src path reads as the `$HOME`-relative target.

macOS and Debian use zsh as login shell with starship prompt. macOS uses Homebrew zsh/plugins under `/opt/homebrew`. Debian uses `/usr/bin/zsh` and distro zsh plugin paths when installed.
...
- Non-interactive package-update confirmation override: `DOTFILES_DEBIAN_UPDATED=1 ./install.sh`.
- OS override for targeted platform-path testing: `DOTFILES_OS=macos|debian ./install.sh`. This is not a dry-run; it still performs installs/links for the selected path.
...
- Secrets scan: the pre-commit hook runs `gitleaks git --pre-commit --staged --redact --verbose`; ad hoc full history audit is `gitleaks git --redact --verbose`. CI runs secret scanning on push via `.github/workflows/gitleaks.yaml`.
...
- `install/packages.sh` — package-manager helpers for apt and optional apt packages.
...
- `LINKS_ZSH`: `.zshrc`, `.zsh/`, tmux, tmux-sessionizer, and starship. Used by macOS and Debian.
...
- macOS detection via `$OSTYPE`; Debian detection via `/etc/os-release` `ID=debian` or Debian-like `ID_LIKE`.
